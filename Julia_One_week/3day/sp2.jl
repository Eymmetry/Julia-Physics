...
function check_collision!(small::Ball, large::Ball,timeseries)
    ...
    told = timeseries[end]
    if v > V
        t = ball_collision_time(small,large)
        x += t*v
        X += t*V
        V_next = (2v+(r-1)V)/(r+1)
...
function make_anime(small::Ball,large::Ball,timeseries,N)
    anim = Animation()
    ts = range(0.00001,timeseries[end]+0.1,length=300)
    ...
    for t in ts
        ...
        X += V*dt
        cnt = ith - 1
        plt = plot(x .+ xcirc,ycirc,label = "count = $cnt pi = $(cnt/10^N)",xlims=(0,8),ylims=(-1,1),aspect_ratio=1,legend=:topright)
        plt = plot!(X .+ Xcirc,Ycirc,label=nothing,xlims=(0,8),ylims=(-1,1),aspect_ratio = 1)
        frame(anim,plt)
    end
